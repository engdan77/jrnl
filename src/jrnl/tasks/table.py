...
def day_summaries_to_table(tasks: list[DaySummaryDict]) -> ui.table:
    ...
    {'name': 'total_time', 'label': 'Time (h)', 'field': 'total_time', 'required': True, 'align': 'left', 'sortable': True},
    ...
    rows = [TasksSummary(**t).to_simpler_dict(enforce_hours=True) for t in tasks]
